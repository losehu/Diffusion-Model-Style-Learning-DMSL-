...
scripts/train_go2_jump_track_prior.sh
scripts/train_go2_jump_track_policy.sh
scripts/test_go2_jump_track_policy.sh
```
...
这三个脚本会默认使用 `data/motions/go2/go2_apex_jump_clean.pkl`；如果 clean 文件不存在，会先从 `data/motions/go2/apex_csv/go2_retarget_jump.csv` 自动生成。
